...
function findAccountIdByName(structure: Structure, name: string): any {
    return structure.accounts.find((account) => account.name === name)._id;
}

function findAccountNameById(structure: Structure, id: any): string {
    return structure.accounts.find((account) => account._id === id).name;
}
